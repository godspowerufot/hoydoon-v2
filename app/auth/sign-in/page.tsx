...
if (err?.data?.error === "account is not active") {
  ...
  router.push("/auth/sign-up/verification");
    ...
    const role = "buyer";
     ...
     setIsPasswordValid(false)
      ...
      const res=  await signup({ username,email, password,role, device,region }).unwrap(); 
      dispatch(setUnverifiedEmail(email));
  ...
  return (
    <div className="flex flex-col  lg:hidden items-center  justify-start w-full min-h-screen px-5 py-2 font-bricolage">
      ...
      </Link>  <Image src="/close.svg" alt="close icon" width={20} height={20} />
      </div>
      ...
      <div className="w-full  mt-4 flex flex-col gap-5">
        ...
        {tab === "signup" && (
          <span
            className="text-sm text-[#1E1E1E99] text-center"
          >
    By submitting, I accept Hoydoon’s <span className="text-primary">terms of use.</span>
          </span>
        )}
      </div>
          ...
          <div className=" gap-[4rem]   flex flex-1 flex-col lg:flex-row">
            ...
            <div className=" w-full lg:w-[50%] items-start mt-[3rem] lg:mt-3 2xl:-mt-4  flex flex-col">
              ...
              <span className=" 2xl:mt-[1rem] flex justify-center flex-col font-bricolage items-center w-full ">
                ...
                <div className="2xl:mt-[2rem] mt-[1rem] flex flex-col gap-[1.5rem] lg:gap-[1em] lg:w-[80%] ">
                  <Input
                    label="Email Address"
                    type="text"
