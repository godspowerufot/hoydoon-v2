...
import { setUnverifiedOtp } from "@/store/slices/authslice";

const ForgetOtpVerify = ({ setStep }) => {
  ...
  dispatch(setUnverifiedOtp(code));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setStep(3); // Move to the next step after successful activation
    } catch (err) {
      console.error("Error resending OTP:", err);
    }
          ...
          <div className=" gap-[4rem]   flex flex-1 flex-col lg:flex-row">
            ...
            <div className=" w-full px-4 lg:w-[60%] items-start mt-3 2xl:-mt-4  flex flex-col">
              ...
              <span className=" 2xl:mt-[1rem]  mt-[1rem] flex justify-center flex-col font-bricolage items-center w-full ">
                ...
                <p className="font-light text-gray   lg:pt-[0.6rem]     text-center  w-full lg:max-w-[23rem] text-[13.5px]">
                  ...
                  <div className="flex gap-3">
                    {otp.map((digit, idx) => (
                      <input
                        ...
                        className="max-w-[55px] max-h-[55px] w-[28%] min-h-[57px] text-center bg-white text-xl border border-[#8F8F8F] rounded-[16px] focus:outline-none"
                      />
                    ))}
                  </div>
                  <div
