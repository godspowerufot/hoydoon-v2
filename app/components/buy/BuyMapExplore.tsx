...
export default function BuyMapExplore() {
  const [inputValue, setInputValue] = useState("");
  const [searchLocation, setSearchLocation] = useState("");
  const [isSearching, setIsSearching] = useState(false);

  const { data: saleListings } = useGetAllListingsQuery({ listingType: "sale" });
  const { data: locationListings, isFetching } = useGetAllListingsQuery(
    { location: searchLocation, listingType: "sale" },
    { skip: !searchLocation }
  );

  const defaultMapListings = useMemo(
    () => flattenListings(saleListings?.listings || []).slice(0, 40),
    [saleListings]
  );
  ...
  return (
    <section className="py-14 md:py-20" aria-labelledby="buy-map-heading">
      <HomeContainer>
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] lg:items-center lg:gap-10">
          <div>
            <SectionHeader
              headingId="buy-map-heading"
              eyebrow="Explore on the map"
              title="See what's for sale near you"
              description="Search a neighborhood, city, or area to preview listings on the map before you dive into details."
            />

            <div className="relative mt-2">
              <label htmlFor="buy-map-search" className="sr-only">
            ...
            <p className="mt-4 text-sm text-[#6b7280]">
              {searchLocation
                ? `${mapListings.length} homes found near "${searchLocation}"`
                : `Showing ${defaultMapListings.length} homes across active markets`}
            </p>
          </div>
